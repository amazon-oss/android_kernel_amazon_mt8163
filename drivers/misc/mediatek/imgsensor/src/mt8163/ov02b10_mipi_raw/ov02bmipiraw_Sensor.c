...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV02BMIPI_SENSOR_ID,

	.checksum_value = 0xb7c53a42,          /* 0x6d01485c */

	.pre = {
		.pclk = 16500000,                  /* record different mode's pclk */
		.linelength = 448,                 /* record different mode's linelength */
		.framelength = 1235,               /* record different mode's framelength */
...
static void write_shutter(kal_uint32 shutter)
{
	...
	/* autoflicker:need to avoid 15fps and 30 fps */
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if((gain >= 0x40) && (gain <= 0x3E0))                    /* base gain = 0x40 ,max gain 15.5*0x40 =0x3E0 */
	{
...
static void sensor_init(void)
{
	...
	usleep_range(3000, 3200);                /* need to delay 3 ms */
	write_cmos_sensor(0xfd, 0x00);
	write_cmos_sensor(0xfd, 0x00);
	...
	write_cmos_sensor(0x8e, 0x10);
	write_cmos_sensor(0x90, 0x10);
	write_cmos_sensor(0x92, 0x10);
	...
	write_cmos_sensor(0xfd, 0x01);           /* 30fps, exposure 33.33ms,AGC1x */
	write_cmos_sensor(0x14, 0x00);
	write_cmos_sensor(0x15, 0x0f);
	write_cmos_sensor(0x0e, 0x05);
	write_cmos_sensor(0x0f, 0x7d);
	write_cmos_sensor(0x22, 0x10);
	write_cmos_sensor(0x12, MIRROR);         /* mirror on ,flip on */
	write_cmos_sensor(0xfe, 0x02);
	write_cmos_sensor(0xfd, 0x03);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				PK_INFO("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
	...
	if (imgsensor_info.sensor_id != sensor_id) {
		PK_INFO("Mute on will not init sensor\n");
		return ERROR_SENSOR_CONNECT_FAIL;
	}
			...
			kal_uint16 frame_length)
{
	...
	if (imgsensor.autoflicker_en == KAL_TRUE) {
		...
		if (realtime_fps >= 297 && realtime_fps <= 305) {
			...
			imgsensor.frame_length = (imgsensor.frame_length  >> 1) << 1;
			write_cmos_sensor(0xfd, 0x01);
			write_cmos_sensor(0x05, (imgsensor.frame_length - 0x4c4) >> 8);
			write_cmos_sensor(0x06, (imgsensor.frame_length - 0x4c4) & 0xFF);
		...
		UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		...
		switch (*feature_data_32) {
			case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
				if (copy_to_user((void __user *) wininfo, (void *) &imgsensor_winsize_info[1], sizeof(struct SENSOR_WINSIZE_INFO_STRUCT))) {
					LOG_INF("copy to user failed\n");
					return -EFAULT;
				}
				break;
			case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
				if (copy_to_user((void __user *) wininfo, (void *) &imgsensor_winsize_info[2], sizeof(struct SENSOR_WINSIZE_INFO_STRUCT))) {
					LOG_INF("copy to user failed\n");
					return -EFAULT;
				}
				break;
			case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
				if (copy_to_user((void __user *) wininfo, (void *) &imgsensor_winsize_info[3], sizeof(struct SENSOR_WINSIZE_INFO_STRUCT))) {
					LOG_INF("copy to user failed\n");
					return -EFAULT;
				}
				break;
			case MSDK_SCENARIO_ID_SLIM_VIDEO:
				if (copy_to_user((void __user *) wininfo, (void *) &imgsensor_winsize_info[4], sizeof(struct SENSOR_WINSIZE_INFO_STRUCT))) {
					LOG_INF("copy to user failed\n");
					return -EFAULT;
				}
				break;
			case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			default:
				if (copy_to_user((void __user *) wininfo, (void *) &imgsensor_winsize_info[0], sizeof(struct SENSOR_WINSIZE_INFO_STRUCT))) {
					LOG_INF("copy to user failed\n");
					return -EFAULT;
				}
			break;
