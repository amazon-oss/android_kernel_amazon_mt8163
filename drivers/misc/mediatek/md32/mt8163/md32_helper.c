			     ...
			     loff_t *ppos)
{
	...
	if (g_md32_log_buf == NULL)
		g_md32_log_buf = kmalloc(log_buf_max_len+1, GFP_KERNEL);

	if (log_buf_max_len != last_log_buf_max_len) {
		last_log_buf_max_len = log_buf_max_len;

		if (!g_md32_log_buf) {
			pr_err("[MD32] %s, kmalloc fail, len=0x%08x\n",
...
{
	/* pr_debug("[MD32] Enter buf_full_ipi_handler\n"); */
	...
	if (g_md32_log_buf_2 == NULL)
		g_md32_log_buf_2 = kmalloc(512+1, GFP_KERNEL);
	...
	/* pr_debug("[MD32] Exit buf_full_ipi_handler\n"); */
}
...
static int reboot_load_md32(void)
{
	int i;
	int ret = 0;
	int d_sz, p_sz;
...
static int md32_probe(struct platform_device *pdev)
{
	...
	md32_send_buff = kmalloc((size_t)64 , GFP_KERNEL);
	md32_recv_buff = kmalloc((size_t)64 , GFP_KERNEL);
...
{
	kfree(md32_send_buff);
	kfree(md32_recv_buff);
	kfree(g_md32_log_buf);
	kfree(g_md32_log_buf_2);
